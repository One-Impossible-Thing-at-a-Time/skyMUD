...
async function main() {
  ...
  if (modified) {
    console.warn(
      "You have uncommitted changes. For safety setup-bundles must be run on a clean repository."
    );
    process.exit(1);
  }
  ...
  ranvierJson.bundles = defaultBundles.map((bundle) =>
    bundle.replace(/^.+\/([a-z\-]+)$/, "$1")
  );
